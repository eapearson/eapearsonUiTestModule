package eapearsonUiTestModule::eapearsonUiTestModuleClient;
...
eapearsonUiTestModule::eapearsonUiTestModuleClient
...
sub new
{
    ...
    my $self = {
	client => eapearsonUiTestModule::eapearsonUiTestModuleClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
sub status
{
    ...
    my $url = $self->{url};
    my $result = $self->{client}->call($url, $self->{headers}, {
        method => "eapearsonUiTestModule.status",
        params => \@args,
    });
    if ($result) {
        if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for eapearsonUiTestModule::eapearsonUiTestModuleClient\n";
    }
    if ($sMajor == 0) {
        warn "eapearsonUiTestModule::eapearsonUiTestModuleClient version is $svr_version. API subject to change.\n";
    }
}
...
package eapearsonUiTestModule::eapearsonUiTestModuleClient::RpcClient;
